    ...
    if uploaded_file:
        ...
        with col2:
            ...
            for i, (day, val) in enumerate(forecast):
                ...
                status_short = status.replace(" for Sensitive Groups", "")
                bg = "#C8E6C9" if val <= 50 else "#FFE0B2" if val <= 100 else "#FFCDD2"
                with cols[i]:
                    st.markdown(
                        f"<div style='background:{bg}; padding:12px; border-radius:12px; text-align:center; box-shadow: 0px 0px 12px {bg}; height:160px; display:flex; flex-direction:column; justify-content:center; align-items:center;'>"
                        f"<p style='margin:0; padding:0; font-weight:bold; font-size:16px;'>{day}</p>"
                        f"<h2 style='margin:6px 0; padding:0; font-size:32px;'>{val}</h2>"
                        f"<p style='margin:0; padding:0; font-size:11px; line-height:1.2;'>{status_short}</p></div>",
                        unsafe_allow_html=True
                    )
